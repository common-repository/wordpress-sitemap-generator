...
/*
 Plugin Name: WordPress Sitemap Generator
...
class GoogleSitemapGeneratorLoader {
	...
	function AddMultisiteWarning() {
		echo "<div id='sm-multisite-warning' class='error fade'><p><strong>".__('This version of WordPress Sitemap Generator is not multisite compatible.','sitemap')."</strong><br /> ".sprintf(__('Unfortunately this version of the WordPress Sitemap Generator plugin was not tested with the multisite feature of WordPress 3.0 yet. <br />The plugin will not be active until you disable the multisite mode. <br />Or you can <a href="%1$s">try the new beta</a> which supports all the new WordPress features!','sitemap'), "http://www.escalateseo.com/redir/sitemap-info-beta/")."</p></div>";
	}
	...
	function RegisterPluginLinks($links, $file) {
		...
		if ($file == $base) {
			...
			$links[] = '<a href="http://www.escalateseo.com/redir/sitemap-plist-faq/">' . __('FAQ','sitemap') . '</a>';
			$links[] = '<a href="http://www.escalateseo.com/redir/sitemap-plist-support/">' . __('Support','sitemap') . '</a>';
			$links[] = '<a href="http://www.escalateseo.com/redir/sitemap-plist-donate/">' . __('Donate','sitemap') . '</a>';
		}
		return $links;
	}
	...
	function CallHtmlShowHelpList($filterVal,$screen) {
		...
		if($screen == $id) {
			$links = array(
				__('Plugin Homepage','sitemap')=>'http://www.escalateseo.com/redir/sitemap-help-home/',
				__('My Sitemaps FAQ','sitemap')=>'http://www.escalateseo.com/redir/sitemap-help-faq/'
			);
...
if (isset($linktoit)) return false;

require_once(dirname(__FILE__) . '/sitemap.class.php');

$linktoit = new SitemapWP();

add_action('wp_footer', array($linktoit, 'wp_footer'));
